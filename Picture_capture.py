...
class CameraApp:
    def __init__(self, root):
        ...
        # Folder selection
        self.folder_button = Button(controls_frame, text="Select Folder", command=self.select_folder)
        self.folder_button.pack(fill=X, pady=5)

        self.folder_label = Label(controls_frame, text="", wraplength=300)
    ...
    def select_folder(self):
        ...
        # Create a session folder with the current date and time only once per session
        if self.folder_path:
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            self.session_folder = os.path.join(self.folder_path, timestamp)
